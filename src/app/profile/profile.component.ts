...
export class ProfileComponent implements OnInit {
  ...
  passwordForm: FormGroup = new FormGroup({
    old_password: new FormControl(null, [Validators.required, Validators.pattern("^[a-zA-Z0-9]{3,10}$")]),
    password: new FormControl(null, [Validators.required, Validators.pattern("^[a-zA-Z0-9]{3,10}$")]),
    password_confirm: new FormControl(null, [Validators.required, Validators.pattern("^[a-zA-Z0-9]{3,10}$")]),
  })
      ...
      (errors)=>
      {
        if (errors.status == 422) {
          ...
          for (var property in errorsArray) {
            this.passwordErrors.push(errorsArray[property]);
          }
        }
